...
export default function SocialLogsPage() {
    ...
    const loadStock = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/social-log/stock?productIds=${encodeURIComponent(ids.join(','))}`,
          { cache: 'no-store' }
        );

        const result = await response.json();

        if (result.status && result.stock) {
        ...
        console.error('Unable to load Social Log stock:', error);
        setStockByProduct({});
        setStockError(true);
      } finally {
  ...
  const refreshProductStock = async (productId: string) => {
    try {
      const response = await fetch(
        `/api/social-log/stock?productIds=${encodeURIComponent(productId)}`,
        { cache: 'no-store' }
      );
      ...
      if (result.status && result.stock) {
        setStockByProduct((prev) => ({
          ...prev,
          [productId]: Number(result.stock[productId] ?? 0)
        }));
        setStockError(false);
      }
    } catch (error) {
      console.error('Unable to refresh Social Log stock:', error);
    }
  };
  ...
  const handlePaystackPayment = async () => {
    ...
    try {
      ...
      await refreshProductStock(selectedProduct.id);
    } catch (error: any) {
      console.error("Social Log wallet purchase error:", error);

      // Refresh stock in case another customer bought the last item.
      ...
      {loadingLogs ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {logs.map((log: any) => {
            ...
            const hasStockData = Object.prototype.hasOwnProperty.call(
              stockByProduct,
              log.id
            );
            const stock = stockByProduct[log.id] ?? 0;
            const isSoldOut = !loadingStock && !stockError && hasStockData && stock <= 0;
            const canBuy = !loadingStock && !stockError && hasStockData && stock > 0;
            ...
            return (
              ...
              >
                <div className="relative aspect-video bg-muted">
                  ...
                  {isSoldOut && (
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                      <Badge className="bg-red-600 text-white font-black text-2xl px-6 py-3 shadow-2xl border-2 border-white">
                        SOLD OUT
                      </Badge>
                    </div>
                  )}
                ...
                <CardContent className="space-y-4 pt-4 mt-auto">
                  <div className={`text-center font-black text-sm rounded-xl py-3 border ${
                    loadingStock
